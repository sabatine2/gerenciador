	...
	private void addGeneratedColumnsToTable() {
		...
		table.addGeneratedColumn("botao",
			...
			public Object generateCell(final Table source, Object itemId,
				...
				Button detailsField = new Button("Registrar");
				...
				detailsField.addListener(new Button.ClickListener() {
					public void buttonClick(ClickEvent event) {
						...
						String redeSocial = (String) source
								.getItem(itemId)
								.getItemProperty(
										ResultadoBusca.REDE_SOCIAL)
										.getValue();
						...
						try {
							com.midiasocial.model.ResultadoBusca res = new com.midiasocial.model.ResultadoBusca();
							res.setDataCriacao(new Date());
							res.setDataCriacaoMidia(data);

							if (url != null) {
								res.setFotoUrl(url.toString());
							}

							res.setMensagem(mensagem);
							res.setNomeUsuario(usuario);

							if (redeSocial != null) {
								res.setRedeSocial(redeSocial);
							}

							if(res.salvar())
								getWindow().showNotification("Registro");	
							else
								getWindow().showNotification("Erro ao registrar ");		

						} catch (Exception e) {
							getWindow().showNotification("Erro ao registrar " + e.getMessage());		
						}
